...
• *ᴇxᴘɪʀᴇᴅ:* ${(xteam.response.expired).replace('Premium left:', '')}`, m)
            ...
            break
          case 'lolhuman': 
    let lol = await (await fetch(`https://api.lolhuman.xyz/api/checkresi?apikey=SGWN&resi=${args[0]}`)).json()
    m.reply(cek)
    if (lol.message == 'success') {
    conn.reply(m.chat, `• *ᴛʏᴘᴇ:* LOLHUMAN
• *ᴀᴘɪᴋᴇʏ:* ${args[0]}

• *ɴᴀᴍᴇ:* ${lol.result.resi}
• *ᴛᴏᴛᴀʟ ʜɪᴛ:* ${lol.result.courier}
• *ʜɪᴛ ᴛᴏᴅᴀʏ:* ${lol.result.history}
