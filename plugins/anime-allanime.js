...
let cita = await pickRandom(res)
await conn.sendButton(m.chat, `${command}`, wm, cita, [['Next', `${usedPrefix + command}`]], m)
}
if (command == 'akiyama') {
...
await conn.sendButton(m.chat, `${command}`, wm, json, [['Next', `${usedPrefix + command}`]], m)
}
if (command == 'yatogami') {
