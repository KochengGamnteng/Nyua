...
let ps = groupMetadata.participants.map(v => v.id)
  ...
  try {
    ...
    var pacar = global.db.data.users[user].pasturi
    var spac = global.db.data.users[m.sender].pasturi
    if(global.db.data.users[m.sender].pasturi != "" && global.db.data.users[global.db.data.users[m.sender].pasturi].pasturi == m.sender && global.db.data.users[m.sender].pasturi != user){
      conn.reply(m.chat, `Kamu sudah menikah dengan @${global.db.data.users[m.sender].pasturi.split('@')[0]}\n\nSilahkan putus dulu (ketik .putusnikah untuk memutuskan hubungan) untuk menembak @${user.split('@')[0]}\n\nBtw yang setia dikit banget!`, m , { contextInfo: { mentionedJid: [user, global.db.data.users[m.sender].pasturi]}})
    }else if(global.db.data.users[user].pasturi != ""){
      if (pacar){
        if (m.sender == pacar && global.db.data.users[m.sender].pasturi == user) return conn.reply(m.chat, `Anda sudah berpacaran dengan ${spac.split('@')[0]}`, m , { contextInfo: { mentionedJid: [spac]}})
        conn.reply(m.chat, `Maaf, @${user.split('@')[0]} sudah mempunyai istri dengan @${pacar.split('@')[0]}\nSilahkan cari jodoh lain!`, m , { contextInfo: { mentionedJid: [user, pacar]}})
      }else{
        global.db.data.users[m.sender].pasturi = user
        conn.reply(m.chat, `${await ktnmbk.getRandom()}\n\nAnda baru saja mengajak @${user.split('@')[0]} menikah\n\nSilahkan menunggu jawaban darinya!\n\nKetik *${usedPrefix}terimanikah @user* untuk menerima\n*${usedPrefix}tolaknikah @user untuk menolak*`, m , { contextInfo: { mentionedJid: [user]}})
      }
    }else if (global.db.data.users[user].pasturi == m.sender){
      global.db.data.users[m.sender].pasturi = user
      conn.reply(m.chat, `Selamat anda sudah menikah dengan @${user.split('@')[0]}\n\nSemoga langgeng dan bahagia selalu dan cepat mempunyai keturunan 🥳🥳🥳`, m , { contextInfo: { mentionedJid: [user]}})
    }else {
      global.db.data.users[m.sender].pasturi = user
      conn.reply(m.chat, `ʜᴀᴘᴘʏ ᴡᴇᴅᴅɪɴɢ @${user.split('@')[0]}\n\nᴋᴀᴍᴜ ᴀᴋᴀɴ ᴍᴇɴɪᴋᴀʜɪ ᴡᴀɴɪᴛᴀ @${user.split('@')[0]}?\n\nsɪʟᴀᴋᴀɴ ᴛᴜɴɢɢᴜ ᴍᴇᴍᴘᴇʟᴀɪ ᴡᴀɴɪᴛᴀ ᴍᴇɴɪʀɪᴍᴀ ᴍᴜ ᴀᴛᴀᴜ ᴛɪᴅᴀᴋ\n\nᴋᴇᴛɪᴋ *${usedPrefix}ᴛᴇʀɪᴍᴀɴɪᴋᴀʜ @user* ᴜɴᴛᴜᴋ ᴍᴇɴᴇʀɪᴍᴀ ᴀᴊᴀᴋ ɴɪᴋᴀʜ\n*${usedPrefix}ᴛᴏʟᴀᴋɴɪᴋᴀʜ @user ᴜɴᴛᴜᴋ ᴍᴇɴᴏʟᴀᴋ ᴀᴊᴀᴋᴀɴ ɴɪᴋᴀʜ*`, m , { contextInfo: { mentionedJid: [user]}})
    }
	}	
}
handler.help = ['nikah *@tag*']
handler.tags = ['jadian']
handler.command = /^(nikah)$/i
handler.group = true
