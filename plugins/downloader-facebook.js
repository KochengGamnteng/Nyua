...
try {
    ...
    if (!args[1]) return conn.sendButton(m.chat, `*${htki} sɪʟᴀᴋᴀɴ ᴘɪʟɪʜ ʀᴇsᴏʟᴜsɪ ${htka}*`, null, null, [['sᴅ', `.fb ${args[0]} sd`],['ʜᴅ', `.fb ${args[0]} hd`]],m)
   let res = await fetch(`https://api.lolhuman.xyz/api/facebook?url=${args[0]}&APIKEY=SGWN`)
...
━━━━━•─────────────── ${duration}
	...
	if (!res) throw 'Tidak bisa mendownload postingan tersebut'
